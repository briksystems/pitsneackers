...
const JORDAN_EDITIONS = ['Retro', 'Travis Scott', 'J Balvin', 'Off-White', 'Trophy Room', 'Fragment x Union', 'A Ma Maniere', 'Nigel Sylvester'];
const KNOWN_EDITION_CODES = {
  Retro: '', 'Travis Scott': 'TRV', 'J Balvin': 'BLV', 'Off-White': 'OW',
  'Trophy Room': 'TR', 'Fragment x Union': 'FXU', 'A Ma Maniere': 'AMM', 'Nigel Sylvester': 'NS',
};
...
/* Tiles grandes y chicos de navegación (inspirados en el grid de Thursday
   Boot Co que mandaste). "image" usa fotos que ya tenemos en el catálogo;
   si no hay foto todavía, el tile queda en un color sólido con el texto. */
const CATEGORY_TILES_BIG = [
  { label: 'Nike', image: 'images/dunk-sb-jarritos.jpg', brand: 'Nike' },
  { label: 'Jordan', image: 'images/jordan1-lost-and-found.jpg', brand: 'Jordan' },
];

const CATEGORY_TILES_SMALL = [
  ...
  { label: 'J Balvin', image: '', brand: 'Jordan', edition: 'J Balvin' },
  { label: 'A Ma Maniere', image: 'images/jordan3-amamaniere-black.jpg', brand: 'Jordan', edition: 'A Ma Maniere' },
...
function ProductDetailModal({ group, onClose, onOrder }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" style={{ background: 'rgba(10,10,10,0.75)' }}>
      <div className="w-full sm:max-w-lg max-h-[90vh] overflow-y-auto rounded-t-2xl sm:rounded-md" style={{ background: ORANGE }}>
        ...
        <div className="p-5">
          <p className="text-[11px] tracking-widest uppercase font-extrabold" style={{ color: CREAM }}>{group.brand}</p>
          <h2 className="text-2xl font-extrabold leading-tight" style={{ color: WHITE, fontFamily: "'Baloo 2', sans-serif" }}>
            {group.model}{group.variant && group.variant !== 'Standard' ? ` ${group.variant}` : ''}
          </h2>
          {group.edition && group.edition !== 'Retro' && <p className="text-sm font-bold mb-1" style={{ color: TEAL }}>x {group.edition}</p>}
          {group.colorway && <p className="text-sm mb-1" style={{ color: CREAM }}>{group.colorway}</p>}
          <p className="text-xs font-semibold mb-3" style={{ color: CREAM }}>{selected.condition || 'Nuevo'} · con caja</p>

          <p className="text-xs font-bold uppercase mb-2" style={{ color: WHITE }}>Elige tu talla</p>
          <div className="flex flex-wrap gap-2 mb-4">
            {sorted.map((v) => (
              <button
                ...
                style={v.id === selectedId ? { background: TEAL, color: WHITE, borderColor: TEAL } : { background: WHITE, color: BLACK, borderColor: WHITE }}
              >
                {v.size}
              </button>
            ))}
          ...
          <p className="text-xs" style={{ color: CREAM }}>Código de autenticación</p>
          <p className="font-mono text-sm mb-3 flex items-center gap-1" style={{ color: WHITE }}><TagIcon size={12} /> {selected.serial}</p>

          <p className="text-3xl font-extrabold mb-1" style={{ color: WHITE, fontFamily: "'Baloo 2', sans-serif" }}>
            {fmtCOP(selected.price)}
          </p>
          {!verified && (
            <p className="text-xs font-semibold mb-4" style={{ color: '#FFE082' }}>
    ...
    <div style={{ background: ORANGE, minHeight: '100%' }}>
      ...
      <div className="flex items-center justify-between px-5 py-4 max-w-6xl mx-auto">
        <div className="flex items-center gap-3">
          <div className="rounded-full flex items-center justify-center overflow-hidden shrink-0" style={{ width: 88, height: 88, background: WHITE }}>
            <img src={MASCOT_SRC} alt="Mascota Pitsneakers" className="w-[90%] h-[90%] object-contain" />
          </div>
          <div>
            <h1 className="text-2xl md:text-3xl tracking-tight" style={{ color: WHITE, fontFamily: "'Titan One', sans-serif" }}>PITSNEAKERS</h1>
            <p className="text-[11px] uppercase tracking-widest" style={{ color: CREAM }}>Sneakers Dealers</p>
          </div>
        </div>
        <button onClick={goAdmin} className="text-xs font-bold flex items-center gap-1 px-3 py-2 rounded-md" style={{ border: `1.5px solid ${WHITE}`, color: WHITE }}>
      ...
      <div className="px-5 max-w-6xl mx-auto pb-8">
        <p className="text-3xl md:text-5xl leading-[1.05] max-w-2xl" style={{ color: WHITE, fontFamily: "'Titan One', sans-serif" }}>
          La comunidad que está cambiando el sneaker game en Colombia.
        </p>
        <p className="mt-3 max-w-xl text-sm md:text-base" style={{ color: CREAM }}>
      ...
      <div className="px-5 max-w-6xl mx-auto pb-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
          {CATEGORY_TILES_BIG.map((t) => (
            <CategoryTile key={t.label} tile={t} big onClick={() => { setBrandFilter(t.brand); setSubFilter(null); }} />
          ))}
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          {CATEGORY_TILES_SMALL.map((t) => (
            <CategoryTile
              key={t.label}
              tile={t}
              onClick={() => { setBrandFilter(t.brand); setSubFilter(t.edition ? { type: 'edition', value: t.edition } : null); }}
            />
          ))}
        </div>
        <button
          onClick={() => { setBrandFilter('Todas'); setSubFilter(null); }}
        ...
        <div className="flex flex-wrap gap-3 items-center">
          <div className="relative">
            ...
            <input
              ...
              className="pl-7 pr-3 py-1.5 rounded-full text-sm" style={{ border: `1px solid ${GREY_LINE}`, background: WHITE }}
            />
          </div>
          <input
      ...
      <div className="px-5 max-w-6xl mx-auto pb-14">
        {groupedProducts.length === 0 ? (
          <p className="text-center py-16 text-sm" style={{ color: CREAM }}>No hay pares que cumplan con esos filtros por ahora.</p>
        ) : (
